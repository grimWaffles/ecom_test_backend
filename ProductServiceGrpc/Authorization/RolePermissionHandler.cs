...
namespace ProductServiceGrpc.Authorization
{
    ...
    public class RolePermissionHandler : AuthorizationHandler<RolePermissionRequirement>
    {
        ...
        public RolePermissionHandler(ILogger<RolePermissionHandler> logger, ITokenHelper tokenHelper, IPermissionService permissionService)
        {
            _logger = logger;
        ...
        protected override async Task HandleRequirementAsync(AuthorizationHandlerContext context, RolePermissionRequirement requirement)
        {
            try
            {
                ...
                bool isAuthorized = await _permissionService.CheckRoleIdAndPermission(Convert.ToInt32(roleId), requirement.Permission);
